...
#
# Usage:
#   ./scripts/dev-up/dev-up.sh                       # boot everything, then the post-boot menu
#   ./scripts/dev-up/dev-up.sh --no-mobile           # skip Expo (menu still runs, minus simulator)
#   ./scripts/dev-up/dev-up.sh --no-frontend         # skip the web app
#   ./scripts/dev-up/dev-up.sh --no-webpage          # skip the public marketing site
#   ./scripts/dev-up/dev-up.sh --no-backend          # skip the backend API
#   ./scripts/dev-up/dev-up.sh --no-ponder           # skip the indexer
#   ./scripts/dev-up/dev-up.sh --skip-db-clone       # reuse previously cloned local databases
#   ./scripts/dev-up/dev-up.sh --mobile-branch <b>   # pull the mobile app from branch <b>
#                                     # (overrides MOBILE_APP_BRANCH in .dev.env)
#   ./scripts/dev-up/dev-up.sh menu                  # dev utilities (set admin, set/clear user
...
# ----------------------------------------------------------------------------
# Dev utilities menu (./scripts/dev-up/dev-up.sh menu) — does not boot any services. Operates
...
run_menu() {
  ...
  "${PSQL[@]}" -d "$APP_DB" -c "SELECT 1" >/dev/null 2>&1 \
    || die "local app database '$APP_DB' not reachable — run ./scripts/dev-up/dev-up.sh first to clone it"
  while true; do
    echo
...
# `./scripts/dev-up/dev-up.sh menu` run on a machine without foundry/go/node and without the
...
open_ios_simulator() {
  ...
  if ! xcrun simctl listapps booted 2>/dev/null | grep -q "host.exp.Exponent"; then
    ...
    if [[ -n "$cached" ]]; then
      ...
      c_red "  One-time setup: cd tmp/mobile-app/mobile && npx expo start --ios (then re-run ./scripts/dev-up/dev-up.sh)"
      return
    fi
  fi
