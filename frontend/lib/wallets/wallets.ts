...
export class AppWallet {
  ...
  // cashOut unwraps `amount` of SFLUV via withdrawTo, burning the SFLUV and
  // sending the underlying USDC 1:1 to `to` (a Bridge liquidation address that
  // settles to the merchant's bank account). Replaces the legacy PayPal
  // Stargate bridge route. Requires REDEEMER_ROLE. Smart wallets only.
  cashOut = async (amount: bigint, to: Address): Promise<TxState | null> => {
    const t = this._beforeTx()
    if (!t) return null

    if (amount <= 0n) {
      return { sending: false, error: "Cash out amount must be greater than zero", hash: null }
    }

    if (!to.startsWith("0x") || to.length !== 42) {
      return { sending: false, error: "Invalid liquidation address", hash: null }
    }

    const walletHasRedeemerRole = await this._hasRedeemerRole(t.wallet.address)
    if (walletHasRedeemerRole === null) {
      return { sending: false, error: "Unable to verify REDEEMER_ROLE on SFLUV contract", hash: null }
    }
    if (!walletHasRedeemerRole) {
      return { sending: false, error: "Wallet is missing REDEEMER_ROLE and cannot cash out", hash: null }
    }
    ...
    if (currentBalance < amount) {
      return { sending: false, error: "Insufficient SFLUV balance for cash out", hash: null }
    }
