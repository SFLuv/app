...
# Round 7 — Aug 27 (evening) – Aug 28

**Repos:** `app` · **Total active hours: 7.6 — provisional**

Merchant onboarding and the location request flow, reworked around a split
between merchant accounts and personal ones. Some of that split already existed
(`users.account_type`, the read-only onboarding gate, the merchant wall); this
round finished it and rebuilt the intake form on top of it.

The figure is **provisional and not from commit clustering**: the round is
entirely uncommitted, so there are no timestamps to cluster and the convention's
usual method does not apply — the same situation Round 2 was in, and it is
labelled the same way. It is derived from volume and the shape of the work: 22
files changed (+1,078/−281), eight new files totalling ~1,000 lines, one
migration, four new routes, and one ~1,050-line form component built from
nothing. It asks to be re-measured from commit timestamps once committed, with
Round 2's caveat attached — if it sits uncommitted for days, the landing
session's timestamps will not be the doing.

## Features

| Feature | hours | repo |
|---|---|---|
| **Location Approval Form as a three-step stepper** — Public Information, Contact, Payment System, each validated on its own before Next will move, with a submitted-successfully screen offering "Apply for another location" and "View my locations". Replaces the single sheet at all three entry points; the sheet stays only for editing a listing that already exists | 2.4 | app |
| **Locations schema and write path reshaped to the questions actually asked** — migration 1.50 adds `contact_name`, `referral_source`, `accepts_tips`, `has_staff_tablet` and backfills the first three; the seven single-sheet columns are kept but no longer collected | 1.5 | app |
| **Merchant/personal account split finished** — signup's policy gate split into two views (policies, then account type); merchant switch moved into Settings both ways; the revert refused server-side in one statement while any location is live or queued; one-time navbar offer for accounts created on the mobile app, keyed off a new `account_type_selected_at` | 1.6 | app |
| **Wallet provisioning split into payment and tipping halves** — a tipping wallet is minted only when the merchant answered yes to tips, and a first location can now inherit the primary wallet for takings while still being minted a tipping wallet. Provisioning returns to approval time (`LOCATION_WALLET_MINT_ON_CREATE` now defaults off) | 0.9 | app |
| **Cancelling a pending application** — owner-scoped soft delete guarded on `approval IS NULL` inside the UPDATE, retiring hours and payment wallets with it; offered on the onboarding page, the locations page and settings | 0.7 | app |
| **Admin review modal shows the new answers** — Contact Name/Email/Phone, how they heard about SFLuv, and a Payment System block that spells out what the tips answer will do at approval | 0.3 | app |

## Smaller fixes

| Fix | hours | repo |
|---|---|---|
| `POST /locations` returns the created id instead of the string "success", so the stepper can attach optional opening hours | 0.1 | app |
| The place finder's manual-entry link became a "Can't find my location" checkbox | 0.1 | app |
...
- **Provisioning moved back to approval, reversing Round 6's direction.** It was
