...
func main() {
	...
	if err := redeemer.SyncWalletRedeemerStatuses(ctx); err != nil {
		appLogger.Logf("error syncing wallet redeemer statuses on startup: %s", err)
	}
	redeemer.StartRoleWatcher(ctx)
