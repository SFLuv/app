...
#
#   ./testing/scripts/seed-w9-tier.sh <address|did> notice_400
#   ./testing/scripts/seed-w9-tier.sh --fast <address|did> blocked
#   ./testing/scripts/seed-w9-tier.sh --revert <address|did>
set -euo pipefail
...
[[ -n "$ev" && "$ev" != "null" ]] || { echo "could not create the event — is the faucet funded? (./testing/scripts/fund-faucet.sh)" >&2; exit 1; }
code="$(code_for "$ev")"
