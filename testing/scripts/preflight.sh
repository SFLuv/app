#!/usr/bin/env bash
# Proves the stack is up, local, and in a state where a test run means something.
#
# Run this first. Every failure here is a reason the rest of the suite would
# produce misleading results rather than useful ones.
...
check_port(){
  ...
  if [[ "$code" =~ ^(200|301|302|307|308|401|403|404)$ ]]; then
    ...
    skip "$name not running at $url — scenarios needing it will skip"
  fi
}
...
# --- authentication -------------------------------------------------------
# Most scenarios act as a real user. Prank forwarding redirects an existing
# identity; it cannot create one, so a captured token is not optional for them.
if [[ -f "$TOKEN_FILE" ]]; then
  body="$(api GET /users)"
  if [[ "$(status)" == "200" ]]; then
    did="$(printf '%s' "$body" | jq -r '.user.id // empty' 2>/dev/null)"
    pass "captured token is valid${did:+ (as $did)}"
    [[ -n "$did" ]] && info "export SFLUV_PRANKER_DID=$did"
  else
    fail "captured token rejected (HTTP $(status)) — re-run capture-token.sh; Privy access tokens last about an hour"
  fi
else
  skip "no captured token — run ./capture-token.sh; API scenarios will skip"
fi
  ...
  if [[ -n "$started_epoch" && -n "$commit_epoch" ]]; then
    if [[ "$started_epoch" -lt "$commit_epoch" ]]; then
      ...
      if [[ $age_min -lt 60 ]]; then
        ...
        skip "the last backend commit is ${age_min}m newer than the process — fine if you just committed code that was already running, otherwise run ./restart-backend.sh"
      else
        fail "the running backend predates the newest backend commit by ~$(( age_min / 60 ))h — run ./restart-backend.sh, or you will debug code that is not running"
      fi
    else
...
# --- chain state ----------------------------------------------------------
# A faucet with no balance is the single most common reason a payout scenario
...
if [[ -n "${SFLUV_FAUCET_ADDRESS:-}" && -n "${SFLUV_TOKEN_ADDRESS:-}" ]]; then
  ...
  if [[ "${bal:-0}" =~ ^[0-9]+$ ]] && [[ "$bal" -gt 0 ]]; then
    ...
    fail "faucet balance is zero — payout scenarios will fail for the wrong reason. Fund it (see the skill)"
  fi
else
...
if [[ -n "$chain_ts" ]]; then
  ...
  if [[ ${drift#-} -lt 300 ]]; then
    ...
    fail "chain clock is ${drift}s out — account abstraction will fail with AA32. Run ./sync-chain-time.sh"
  fi
fi

# --- W-9 enforcement ------------------------------------------------------
# Shadow mode computes the decision and pays anyway, so every W-9 gate scenario
# would pass for the wrong reason.
info "W9_ENFORCEMENT must be 'enforce' locally or the W-9 scenarios prove nothing"
